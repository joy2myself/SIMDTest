...
static void BM_Mandel_scalar(benchmark::State& state) {
    ...
    for (auto _ : state)
        scalar::mandelbrot(x_0, y_0, x_1, y_1, _width, _height, _maxIters, _buf.data());
}//scalar bench function

template <int N>
static void BM_Mandel_mysimd(benchmark::State& state) {
    std::fill(_buf.begin(), _buf.end(), 0);
    for (auto _ : state)
        mysimd::mandelbrot<N>(x_0, y_0, x_1, y_1, _width, _height, _maxIters, _buf.data());
...
static void BM_Mandel_VCL(benchmark::State& state) {
    std::fill(_buf.begin(), _buf.end(), 0);
    for (auto _ : state)
        VCL::mandelbrot<_Tp,_Tpb,_Tpi>(x_0, y_0, x_1, y_1, _width, _height, _maxIters, _buf.data());
}//VCL bench function
...
static void BM_Mandel_xsimd(benchmark::State& state) {
    std::fill(_buf.begin(), _buf.end(), 0);
    for (auto _ : state)
        x_simd::mandelbrot<arch>(x_0, y_0, x_1, y_1, _width, _height, _maxIters,_buf.data());
...
BENCHMARK_TEMPLATE(BM_Mandel_mysimd, 4);
BENCHMARK_TEMPLATE(BM_Mandel_mysimd, 8);
